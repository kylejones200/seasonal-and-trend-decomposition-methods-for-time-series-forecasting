...
def generate_synthetic_data(start_date: str='2024-01-01', end_date: str='2025-12-31', freq: str='D', seed: int=42) -> pd.DataFrame:
...
def analyze_components(decomposition_result) -> dict[str, Any]:
    ...
    return {'trend_direction': trend_direction, 'trend_strength': trend_strength, 'seasonal_amplitude': seasonal_amplitude, 'residual_variance': residual_variance}

def plot_decomposition_comparison(df: pd.DataFrame, additive, multiplicative, output_path: Path, plot: bool=False):
    ...
    plt.suptitle('Additive vs Multiplicative Decomposition Comparison', fontsize=12, y=0.98, color='0.2')
